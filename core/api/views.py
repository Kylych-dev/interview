...
def index(request):
    ...
    for usernames in user_following_list:
        feed_lists = Post.ojbects.filter(user=usernames)
    ...
    return render(request, 'index.html', {'user_profile': user_profile,
                                          'posts': feed_list,
                                          'suggestions_username_profile_list': suggestion_username_profile_list[:4]})
...
def search(request):
    ...
    return render(request, '/', {'user_profile': user_profile, 'username_profile': username_profile_list})
...
def upload(request):
    if request.method == 'POST':
        ...
        new_post = Post.objects.creat(user=user, image=image, caption=caption)
        new_post.save()
        return redirect('')
    else:
        return redirect('/')
    # return HttpResponse('<h> upload </h1>')
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                auth.request(request, user_login)
